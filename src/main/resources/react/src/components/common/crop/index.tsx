...
import { deleteAvatarThunk, postAvatarThunk } from 'src/store/profile/thunk';
...
const Crop = ({ image, onClose, setImage, setCroppedImage, disabled }) => {
  ...
  const showCroppedImage = useCallback(async () => {
    try {
      setLoading(true);
      const croppedPicture = await getCroppedImg(image, croppedAreaPixels, rotation);
      ...
      if (typeof croppedPicture?.src === 'string') {
        ...
        const receivedImage = await dispatch(postAvatarThunk(dataForm, croppedPicture.src));
      ...
      <Styles.Container ref={ref}>
        <Styles.Wrap>
          ...
          <Styles.BlockButtons>
            ...
            <Button onClick={onClose} text={getTranslatedText('button.cancel')} />
