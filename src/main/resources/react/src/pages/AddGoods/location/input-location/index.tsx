...
import { area, city, district } from 'src/Utils';
import { DropDownInput } from 'src/pages/AddGoods/drop-down-input';
...
const InputLocation = ({
  ...
  const getLocationId = (elem) => {
    const cityCurr = elem.indexOf('(') === -1 ? elem : elem.substring(0, elem.indexOf('(') - 1);

    const districtCurr =
      elem.indexOf('(') === -1 ? '' : elem.substring(elem.indexOf('(') + 1, elem.indexOf(')'));
      ...
      if (name === city) {
        ...
        return sortAlphabet(
          location
            ...
            .map((item) => (item[district] ? `${item[name]} (${item[district]})` : item[name]))
        );
      }
      return [];
    };
