...
const MySettings = () => {
	...
	const validationEmailSchema = yup.object().shape( {
		newEmail:
			yup.string().notOneOf( [ currentEmail ], 'Email совпадает со старым' )
			...
			>
				{ ( { errors, isValid, handleSubmit, dirty } ) => (
					<>
						...
						<Button
							text={ isFetchPass ? <SpinnerForAuthBtn/> : getTranslatedText( `button.save`, lang ) }
				...
				onSubmit={ (async ( values, onSubmitProps ) => {
					...
					try {
						...
						if (e.response.status === 409) {
							onSubmitProps.setErrors( { newEmailConfirmation: e.response.data.error } );
						}
						if (e.response.status === 400) {
			...
			>
				{ ( {  errors, isValid, handleSubmit, dirty } ) => (
					<>
						...
						<Button
							text={ isFetchEmail ? <SpinnerForAuthBtn/> : getTranslatedText( `button.codeConfirm`, lang ) }
