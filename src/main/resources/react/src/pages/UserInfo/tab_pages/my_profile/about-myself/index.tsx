...
const AboutMyself = () => {
  ...
  const onSubmit = async (dataFormik) => {
    ...
    try {
      ...
      if (!newUserData.lastName && !newUserData.firstName && !newUserData.phones.length) {
      ...
      if (err.response?.status === 400) {
        const indexStart = err.response?.data?.error && err.response.data.error.indexOf(':') + 1;
            ...
            {({ push, remove }) => (
              <Styles.WrapperInputPhones>
                {values.phones?.map((phone, index, arr) => {
                  ...
                  return (
                    <Styles.WrapperInputAddPhones key={String(`phones[${index}]`)}>
                      <InputField
                        type='tel'
                        labelColor='black'
                        inputMaxWidth='588px'
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (!!phone && !errorField) push('');
                                ...
                                showMessage.error(getTranslatedText('ownInfo.choosePhone'));
                              }
                            }}
                          />
                        </Styles.WrapperAddButton>
