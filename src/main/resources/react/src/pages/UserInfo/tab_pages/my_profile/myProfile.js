...
const MyProfile = ({profile}) => {
	...
	return (
		<form onSubmit={submitForm}>
			...
			<InputProfile data={{name: 'firstName', label: 'Имя:', value: profile.firstName, type: 'text'}} click={addInputMe}/>
			<InputProfile data={{name: 'lastName', label: 'Фамилия:', value: profile.lastName, type: 'text'}} click={addInputMe}/>
			<InputProfile data={{name: 'city', label: 'Город:', value: profile.city, type: 'text'}} click={addInputCity} disabled/>

			{profile.phones.map((phone, index) => {
				return <InputProfile
					data={{name: 'phoneNumber', label: `${!index ? 'Телефон:' : ''} `, value: phone.phoneNumber, type: 'phone'}}
					key={index}
				...
				{profile.children.map((child, idx) => {
					return (
						<div className={'block-child'} key={`${child.id}_child`}>
							<InputProfile data={{name: 'sex', label: 'Имя:', value: child.sex, type: 'text'}}
								click={(e) => addInputChildren(e, child.id)}/>
							<InputProfile data={{name: 'birthDate', label: 'Возраст:', value: child.birthDate, type: 'date'}}
								click={(e) => addInputChildren(e, child.id)}/>
