...
import { ModalContext } from 'src/components/common';
import { setProfileEmail } from 'src/store/profile/slice';
import { getAuthProfile, putEmail } from 'src/store/auth/slice';
...
const MySettings = () => {
  ...
  const handleDelete = async (values) => {
    ...
    try {
      ...
      openModal({
        title: <Styles.ModalTitle>{getTranslatedText('settings.removed')}</Styles.ModalTitle>,
        children: <p>{data}</p>,
      });
    } catch (e) {
      showMessage.error(e.response.data.error);
  ...
  const deleteAccountModal = () => {
    openModal({
      title: <Styles.ModalTitle>{getTranslatedText('settings.remove')}</Styles.ModalTitle>,
      children: (
        <div>
  ...
  return (
    <>
      ...
      <Title style={{ margin: '65px 0 40px' }} text={getTranslatedText('settings.changeEmail')} />
      ...
      <Title style={{ margin: '65px 0 40px' }} text={getTranslatedText('settings.remove')} />
